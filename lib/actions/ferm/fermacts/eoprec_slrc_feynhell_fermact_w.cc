// $Id: eoprec_slrc_feynhell_fermact_w.cc,v 1.1 2007-10-09 03:06:51 edwards Exp $
/*! \file
...
namespace Chroma
{
  ...
  namespace EvenOddPrecSLRCFeynHellEnv
  {
    ...
    {
      return new EvenOddPrecSLRCFeynHell(CreateFermStateEnv::reader(xml_in, path), 
    ...
    //! Name to be used
    const std::string name = "EOPREC_SLRC_FEYNHELL";
  ...
  EvenOddPrecLogDetLinearOperator<LatticeFermion,
				  multi1d<LatticeColorMatrix>,
				  multi1d<LatticeColorMatrix> >* 
  EvenOddPrecSLRCFeynHell::linOp(Handle< FermState<T,P,Q> > state) const
  {
    return new EvenOddPrecSLRCFeynHellLinOp(state,param,fhparam);
  }
