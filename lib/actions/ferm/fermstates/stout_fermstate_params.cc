...
namespace Chroma
{
  ...
  MomentumFermStateParams::MomentumFermStateParams()
  {
    rho.resize(Nd, Nd);
    smear_in_this_dirP.resize(Nd);
  ...
  MomentumFermStateParams::MomentumFermStateParams(XMLReader& in, const std::string& path)
  {
    try
      {
        XMLReader paramtop(in, path);
  ...
  {
    MomentumFermStateParams tmp_p(xml, path);
    p = tmp_p;
  }
