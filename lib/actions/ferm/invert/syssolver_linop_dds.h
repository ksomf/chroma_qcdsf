...
namespace Chroma
{
  ...
  public:
	...
	bs[i]=invParam.BS[(i+3)%Nd];
      ...
      QDPIO::cout << "DD Solver Init." << std::endl;
	...
	QMP_barrier();
      ...
      QDPIO::cout << " Loading gauge field in DD Solver done." << std::endl;
      /*
      QDPIO::cout << " Dirac operator test..." << std::endl;
      double kappa = toDouble(invParam.Kappa);
      double csw = toDouble(invParam.Csw);


      QDPIO::cout << " Dirac test done: OK! " << std::endl;
      */
    }
	...
	QDPIO::cout << "DD Solver total time: "<<time<< " sec, res: " << residual << " " << std::endl;
	...
	if((residual/toDouble(res.resid)) < 0.2) {
	  QDPIO::cout << "DDS RESIDUAL CHECK FAILED :( DD: " << residual << "CHK: " << res.resid << std::endl;
	  exit(1);
	} else {
	  QDPIO::cout << "DDS_SOLVER_CHECK: "<< res.resid << " Relative Rsd = " << res.resid/sqrt(norm2(chi,A->subset())) << std::endl;
	}
	END_CODE();
	return res;
    }
