...
namespace Chroma 
{
   ...
   */
  void EvenOddPrecDumbSlrcFLinOpQCDSF::create(Handle< FermState<T,P,Q> > fs, 
   ...
   */
  void EvenOddPrecDumbSlrcFLinOpQCDSF::operator()(T& chi, 
					  const T& psi, 
  ...
  unsigned long EvenOddPrecDumbSlrcFLinOpQCDSF::nFlops() const
  {
  ...
  const Subset& EvenOddPrecDumbSlrcFLinOpQCDSF::subset() const {
    return rb[1];
  }
   ...
   * \param param_  fermion kappa   	       (Read)
   */
  void EvenOddPrecDumbSlrcDLinOpQCDSF::create(Handle< FermState<T,P,Q> > fs, 
					   const CloverFermActParams& param_)
  {
    START_CODE();
   ...
   * \param isign   Flag ( PLUS | MINUS )   	       (Read)
   */
  void EvenOddPrecDumbSlrcDLinOpQCDSF::operator()(T& chi, 
					  const T& psi, 
					  enum PlusMinus isign) const
  {
    START_CODE();
  ...
  //! Return flops performed by the operator()
  unsigned long EvenOddPrecDumbSlrcDLinOpQCDSF::nFlops() const
  {
    unsigned long cbsite_flops = 2*D.nFlops()+2*clov.nFlops()+4*Nc*Ns;
    if(  param.twisted_m_usedP ) { 
      cbsite_flops += 4*Nc*Ns; // a + mu*b : a = chi, b = g_5 I psi
  ...
  const Subset& EvenOddPrecDumbSlrcDLinOpQCDSF::subset() const {
    return rb[1];
  }
