...
namespace Chroma
{
  ...
  //! Momentum version of GaugeState
  /*! @ingroup gaugestates
   *
   * Only needs to hold a gauge field and gauge bc
   */
  template<typename P, typename Q>
  class MomentumGaugeState : public GaugeState<P,Q>
  {
  public:
    //! Full constructor
    MomentumGaugeState(Handle< GaugeBC<P,Q> > gbc_,
		    const MomentumFermStateParams& p_,
		    const Q& q_)
      {
	START_CODE();
	create(gbc_, p_, q_);
...
//         = 3: no   u[3] no   gbc->
      ...
      for(int mu=0; mu < Nd; mu++) {
           ...
           if(mu == 3 && (params.momk[mu] <= 1e-5) )
             continue;
           (smeared_links[0])[mu] = u_[mu]*cmplx(cos(theta),-1.0*sin(theta));
...
////////////////////////////////////////////////////////////////////////////
  ...
  private:
     MomentumGaugeState() {}  // hide default constructur
    void operator=(const MomentumGaugeState&) {} // hide =

  private:
    Handle< GaugeBC<P,Q> >  gbc;
    multi1d<Q> smeared_links;
    MomentumFermStateParams params;
  };
   ...
   */
  template<typename P, typename Q>
  class CreateMomentumGaugeState : public CreateGaugeState<P,Q>
  {
  public:
    //! Full constructor
    CreateMomentumGaugeState(Handle< GaugeBC<P,Q> > gbc_,
			  const MomentumFermStateParams& p_) : gbc(gbc_), params(p_) {}

    //! Destructor
    ~CreateMomentumGaugeState() {}

    //! Construct a ConnectState
    MomentumGaugeState<P,Q>* operator()(const Q& q) const
      {
	return new MomentumGaugeState<P,Q>(gbc, params, q);
      }
  ...
  private:
    CreateMomentumGaugeState() {}  // hide default constructur
    void operator=(const CreateMomentumGaugeState&) {} // hide =
