...
{ 
  ...
  void read(XMLReader& xml, const string& path, InlineBarspecParamsQCDSF::Param_t& param)
  {
  ...
  void write(XMLWriter& xml, const string& path, const InlineBarspecParamsQCDSF::Param_t& param)
  {
    push(xml, path);
  ...
  //! Propagator input
  void read(XMLReader& xml, const string& path, InlineBarspecParamsQCDSF::NamedObject_t::Props_t& input)
  {
  ...
  //! Propagator output
  void write(XMLWriter& xml, const string& path, const InlineBarspecParamsQCDSF::NamedObject_t::Props_t& input)
  {
    push(xml, path);
  ...
  //! Propagator input
  void read(XMLReader& xml, const string& path, InlineBarspecParamsQCDSF::NamedObject_t& input)
  {
    XMLReader inputtop(xml, path);
  ...
  //! Propagator output
  void write(XMLWriter& xml, const string& path, const InlineBarspecParamsQCDSF::NamedObject_t& input)
  {
    push(xml, path);
  ...
  namespace 
  {
    ...
    {
      QDPIO::cout << "3rd id=" << sink_pair.third_id << endl;
				 ...
				 XMLWriter& xml_out)
  {
	...
	if (params.lime_file != "")
	  ...
	  {
	    QDPIO::cerr << "Error!! lime_file must be declared! " << endl;
	    QDP_abort(1);
	  }
      } 
			       ...
			       XMLWriter& xml_out)
  {
    ...
    QDPIO::cout << InlineBarSpecEnvQCDSF::name << ": ran successfully" << endl;
				...
				string& lime_file)
  {
    ...
    QDPIO::cout <<         " BARSPEC-QCDSF: Spectroscopy for Wilson-like fermions" << endl;
    QDPIO::cout << endl << "            Gauge group: SU(" << Nc << ")" << endl;
    QDPIO::cout << "     volume: " << Layout::lattSize()[0];
    for (int i=1; i<Nd; ++i) {
      QDPIO::cout << " x " << Layout::lattSize()[i];
    ...
    QDPIO::cout << "writing LIME QCDSF header" << endl;
	...
	QDP_abort(1);
      ...
      QDPIO::cout << "writing LIME record xml" << endl;
	...
	QDPIO::cout << "writing LIME binary data" << endl;
