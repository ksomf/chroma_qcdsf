...
namespace Chroma
{
	namespace InlineNprVertexNonlocalEnv
	{
		...
		const std::string name = "NPR_VERTEX_NONLOCAL";
		...
		return sum(prop*phases[0])/Double(Layout::vol());
						  ...
						  void read(XMLReader& xml, const std::string& path, InlineNprVertexNonlocalParams::Param_t& input)
						  {
							  ...
							  switch (version)
							  {
								  ...
								  default :
									  QDPIO::cerr << InlineNprVertexNonlocalEnv::name << ": input parameter version "
						  ...
						  // Param stuff
						  InlineNprVertexNonlocalParams::InlineNprVertexNonlocalParams() {frequency = 0;}
						  ...
						  void
						  InlineNprVertexNonlocalParams::write(XMLWriter& xml_out, const std::string& path)
						  {
							  push(xml_out, path);
						  ...
						  // Function call
						  void
						  InlineNprVertexNonlocal::operator()(unsigned long update_no,
															  XMLWriter& xml_out)
						  {
						  ...
						  // Function call
						  void
						  InlineNprVertexNonlocal::func(unsigned long update_no,
														XMLWriter& XmlOut)
						  {
							  START_CODE();
							  ...
							  {
								  QDPIO::cerr << InlineNprVertexNonlocalEnv::name << ": caught dynamic cast error"
								  << std::endl;
								  QDP_abort(1);
							  }
							  catch (const std::string& e)
							  {
								  QDPIO::cerr << InlineNprVertexNonlocalEnv::name << ": std::map call failed: " << e
								  << std::endl;
								  QDP_abort(1);
							  }
							  catch( ... )
							  {
								  QDPIO::cerr << InlineNprVertexNonlocalEnv::name << ": caught generic exception "
								  << std::endl;
								  QDP_abort(1);
							  }
							  ...
							  {
								  QDPIO::cerr << InlineNprVertexNonlocalEnv::name << ": forward prop: error message: " << e
								  << std::endl;
								  QDP_abort(1);
							  }
							  ...
							  QDPIO::cout << "finished calculating NprVertexNonlocal"
							  << "  time= "
							  << swatch.getTimeInSeconds()
							  << " secs" << std::endl;
							  ...
							  snoop.stop();
							  QDPIO::cout << InlineNprVertexNonlocalEnv::name << ": total time = "
