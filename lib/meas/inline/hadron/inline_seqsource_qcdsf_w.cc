...
{ 
  ...
  { 
	...
	QDP_abort(1);
      ...
      multi1d<bool> forward_smeared(params.named_obj.prop_ids.size());
	...
	try
	{
	  ...
	  forward_smeared[loop] = prop_record_xml.count("/SinkSmear") > 0;

// 	  // Sanity check whether all input propagators are either all smeared or all not-smeared
// 	  if (loop)
