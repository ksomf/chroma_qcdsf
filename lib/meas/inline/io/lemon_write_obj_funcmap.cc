// $Id: lemon_write_obj_funcmap.cc,v 3.14 2009-01-30 03:42:40 kostas Exp $
/*! \file
 *  \brief Write object function map
 */

#include "named_obj.h"
#include "meas/inline/io/lemon_write_obj_funcmap.h"
...
namespace Chroma
{
  ...
  { 
    ...
    namespace
    {
      //------------------------------------------------------------------------
      //! Write a propagator
      void LemonWriteLatProp(const string& buffer_id,
			   const string& file, 
	...
	TheNamedObjMap::Instance().get(buffer_id).getRecordXML(record_xml);
        ...
        writeLemonQIOLatProp( file_xml, record_xml, obj, file );
      }


/*      //! Write a single prec propagator
      void LemonWriteLatPropF(const string& buffer_id,
			    const string& file, 
			    QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	...
	close(to);
      ...
      void LemonWriteLatPropD(const string& buffer_id,
			    const string& file, 
			    QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	...
	close(to);
      ...
      //------------------------------------------------------------------------
      //! Write a fermion
      void LemonWriteLatFerm(const string& buffer_id,
			   const string& file, 
			   QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	LatticeFermion obj;
...
*/
      ...
      void LemonWriteLatFermF(const string& buffer_id,
			    const string& file, 
			    QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	LatticeFermionF obj;
	...
	close(to);
      ...
      void LemonWriteLatFermD(const string& buffer_id,
			    const string& file, 
			    QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	LatticeFermionD obj;
...
/*      //------------------------------------------------------------------------
      //! Write a propagator
      void LemonWriteLatStagProp(const string& buffer_id,
			       const string& file, 
			       QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	...
	close(to);
      ...
      //! Write a single prec propagator
      void LemonWriteLatStagPropF(const string& buffer_id,
				const string& file, 
				QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	...
	close(to);
      ...
      //! Write a double prec propagator
      void LemonWriteLatStagPropD(const string& buffer_id,
				const string& file, 
				QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	...
	close(to);
      ...
      void LemonWriteArrayLatColMat(const string& buffer_id,
				  const string& file, 
				  QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	...
	close(to);
      ...
      void LemonWriteArrayLatColMatF(const string& buffer_id,
				   const string& file, 
				   QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	XMLBufferWriter file_xml, record_xml;
	...
	close(to);
      ...
      void LemonWriteArrayLatColMatD(const string& buffer_id,
				   const string& file, 
				   QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	XMLBufferWriter file_xml, record_xml;
	...
	close(to);
      ...
      void LemonWriteQQDiquarkContract(const string& buffer_id,
				     const string& file, 
				     QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	XMLBufferWriter file_xml, record_xml;
	...
	// Lemon cannot deal with multiNd arrays, so we have to flatten to a 
	...
	close(to);
      ...
      template<typename T>
      void LemonWriteEigenInfo(const string& buffer_id,
			     const string& file,
			     QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	// This is needed for Lemon writing
	XMLBufferWriter file_xml, record_xml;
	...
	close(to);
      ...
      void LemonWriteRitzPairsLatticeFermion(const string& buffer_id,
					   const string& file,
					   QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	// A shorthand for the object
	...
	close(to);
      ...
      //----------------------------------------------------------------------
      void LemonWriteSubsetVectors(const string& buffer_id,
				 const string& file,
				 QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	// A shorthand for the object
	...
	close(to);
      ...
      void LemonWriteMapObjMemory(const string& buffer_id,
				const string& file,
				QDP_volfmt_t volfmt, QDP_serialparallel_t serpar)
      {
	// This is needed for Lemon writing
...
*/
    ...
    bool registerAll() 
    {
      ...
      if (! registered)
      {
	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("LatticePropagator"), 
								      LemonWriteLatProp);
/*	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("LatticePropagatorF"), 
								      LemonWriteLatPropF);
	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("LatticePropagatorD"), 
								      LemonWriteLatPropD);

	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("LatticeFermion"), 
								      LemonWriteLatFerm);
//      success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("LatticeFermionF"), 
//								    LemonWriteLatFermF);
//      success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("LatticeFermionD"), 
//								 LemonWriteLatFermD);

	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("LatticeStaggeredPropagator"), 
								      LemonWriteLatStagProp);
	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("LatticeStaggeredPropagatorF"), 
								      LemonWriteLatStagPropF);
	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("LatticeStaggeredPropagatorD"), 
								      LemonWriteLatStagPropD);

	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("Multi1dLatticeColorMatrix"), 
								      LemonWriteArrayLatColMat);
	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("Multi1dLatticeColorMatrixF"), 
								      LemonWriteArrayLatColMatF);
	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("Multi1dLatticeColorMatrixD"), 
								      LemonWriteArrayLatColMatD);

	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("QQDiquarkContract"), 
								      LemonWriteQQDiquarkContract);

	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("EigenInfoLatticeFermion"), 
								      LemonWriteEigenInfo<LatticeFermion>);

	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("RitzPairsLatticeFermion"), 
								      LemonWriteRitzPairsLatticeFermion);
	
	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("SubsetVectorsLatticeColorVector"), 
								      LemonWriteSubsetVectors);

	success &= TheLemonWriteObjFuncMap::Instance().registerFunction(string("MapObjMemoryKeyPropColorVecLatticeFermion"), 
								      LemonWriteMapObjMemory<KeyPropColorVec_t,LatticeFermion>);
*/
	registered = true;
      }
      return success;
